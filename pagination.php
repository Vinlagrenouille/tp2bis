...
<body>
  ...
  <?php
    ...
    $data = array(
      ...
      foreach($data as $array){
				if ($counter > ($current*$parpage) and $counter < (($current*$parpage)+$parpage)){
  ...
  ?>
	...
	<form method="get" action="pagination.php">
     	<?php
				if($current > 0){
        	echo "<a href=\"?current=".($current-1)."\"><input type=\"button\" value=\"Precedent\"></a>";
				}
				for($i = 0; $i < $nbpage;$i = $i +1){
						echo "<a href=\"?current=".$i."\"><input type=\"button\" value=\"".($i+1)."\"></a>";
				}
				if($current < $nbpage-1){
        	echo "<a href=\"?current=".($current+1)."\"><input type=\"button\" value=\"Suivant\"></a>";
				}
      ?>  
    </form>
</body>
